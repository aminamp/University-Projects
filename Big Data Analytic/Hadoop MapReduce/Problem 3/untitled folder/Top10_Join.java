...
public class Top10_Join {
	...
	public static void main(String[] args) throws IOException,
		...
		Job job1 = new Job(conf, "Top 10 Rated Businesses");

		job1.setJarByClass(Top10_Join.class);

		Path inputFile = new Path(otherArgs[0]);    //review.csv
        Path inputFile = new Path(otherArgs[1]);    //bussiness.csv
		Path outputFile = new Path(otherArgs[2]);
		Path intermidiateFile = new Path("axa142831/intermediate_data");
		...
		MultipleInputs.addInputPath(job2, intermidiateFile,TextInputFormat.class, Top10IdentityMapper.class);
		MultipleInputs.addInputPath(job2, inputFile2, TextInputFormat.class, DetailsMapper.class);
    ...
    //write business_id and stars of review.csv
		...
		protected void map(LongWritable baseAddress, Text line, Context context)
			...
			if (fields.length == 4) {
				if (1) {
					...
					stars.set(Integer.parseInt(fields[3].trim()));
